...
async function startSnapshotExport(ctx, args) {
  ...
  try {
    ...
    if (e instanceof import_utils.ThrowingFetchError && e.serverErrorData?.code === "ExportInProgress") {
      (0, import_log.failExistingSpinner)();
      const progressAdvice = args.snapshotExportDashboardLink === void 0 ? "" : `
Check its progress at ${args.snapshotExportDashboardLink}`;
      return await ctx.crash({
        exitCode: 1,
        errorType: "fatal",
        printedMessage: "A snapshot export is already requested or in progress. Only one can run at a time, so wait for it to finish and try again." + progressAdvice
      });
    }

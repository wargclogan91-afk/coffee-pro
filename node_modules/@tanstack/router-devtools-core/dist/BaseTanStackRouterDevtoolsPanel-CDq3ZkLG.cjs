...
//#endregion
const require_context = require("./context-MWhSpdWa.cjs");
let clsx = require("clsx");
let _tanstack_router_core = require("@tanstack/router-core");
let goober = require("goober");
goober = __toESM(goober, 1);
...
function AgeTicker({ match, router }) {
	...
	const gcTime = route.options.gcTime ?? router().options.defaultGcTime ?? 3e5;
	return (() => {
...
function RouteComp({ routerState, pendingMatches, router, route, isRoot, activeId, setActiveId }) {
	const styles = useStyles$1();
	const matches = require_context.createMemo(() => pendingMatches().length ? pendingMatches() : routerState().matches);
...
var BaseTanStackRouterDevtoolsPanel = function BaseTanStackRouterDevtoolsPanel({ ...props }) {
	...
	const pendingMatches = () => {
		...
		return matches.some((match) => match.status === "pending") ? matches : [];
	};
	let cache = router()._cache;
	let cacheMatches = [...cache.values()];
	const [cachedMatches, setCachedMatches] = require_context.createSignal(cacheMatches);
	require_context.createEffect(() => {
		const refreshCache = () => {
			...
			if (!valuesChanged) {
				let index = 0;
				for (const match of next.values()) if (match !== cacheMatches[index++]) {
					valuesChanged = true;
					break;
				}
			}
		...
		require_context.onCleanup(() => clearInterval(interval));
	});
	require_context.createEffect(() => {
		const matches = routerState().matches;
	...
	const hasSearch = require_context.createMemo(() => (0, _tanstack_router_core.hasKeys)(routerState().location.search));
		...
		require_context.insert(_el$30, require_context.createComponent(require_context.Switch, { get children() {
			return [
				...
				require_context.createComponent(require_context.Match, {
					...
					get children() {
						var _el$31 = _tmpl$6();
						require_context.insert(_el$31, () => (pendingMatches().length ? pendingMatches() : routerState().matches).map((match, _i) => {
							return (() => {
		...
		require_context.insert(_el$11, (() => {
			...
			return () => _c$6() ? (() => {
				...
				require_context.insert(_el$65, (() => {
					var _c$9 = require_context.memo(() => !!pendingMatches().find((d) => d.id === activeMatch()?.id));
...
//# sourceMappingURL=BaseTanStackRouterDevtoolsPanel-CDq3ZkLG.cjs.map